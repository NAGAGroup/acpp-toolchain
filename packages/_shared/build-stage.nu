...
# never the stage install dir — CUDA's libdevice and the OpenCL loader are
# host packages.
...
# `libprefix` here is the DEPENDENCY ROOT (%PREFIX%\Library), never the stage
...
def main [] {
  ...
  # THE STAGE SUBDIRECTORY. This build no longer installs to the top of the
  # prefix: it installs into <layout_root>/_stage, and each carving subpackage
  # copies ITS OWN portion up to the top level. Two reasons, both structural:
  #
  ...
  #     already present in its $PREFIX and is therefore NOT new — nothing
  #     would be captured. Installing one level down makes the copy-to-top
  #     BE the diff.
  ...
  #     (which this one has: cmake, ninja, python, zlib, cuda, boost...)
  #     cannot resolve. Measured on pixi 0.78.0 and 0.79.0, green under plain
  #     rattler-build. This design routes around it with ordinary packages.
  #
  ...
  # _stage/include, ...). Binaries are linked with $ORIGIN-relative rpaths
  # ($ORIGIN/../lib), and those survive the copy to the top level ONLY because
  # _stage/bin -> _stage/lib has the same relative relationship as bin -> lib.
  # Copying to a different relative depth would break every binary. The same
  # invariance is what lets the clang driver find its resource directory
